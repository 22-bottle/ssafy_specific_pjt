...
const stories: Storyline[] = [
    ...
    {
      id : 6,
      text :"타임머신의 문을 만지는 순간 타임머신에서 불빛이 나오기 시작했어요",
      image:storyBackgroundLab
    },
    {
      id : 7,
...
const Story:React.FC= () => {
    ...
    return (
      ...
      <div className={styles.storyNavigation}>
        <button onClick={goToPreviousStory} disabled={currentStoryIndex === 0}>
          Previous
        </button>
        <p>{stories[currentStoryIndex].text}</p>
        <button onClick={goToNextStory} disabled={currentStoryIndex === stories.length - 1}>
          Next
        </button>
      </div>
    </div>
